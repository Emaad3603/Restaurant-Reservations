...
try {
    ...
    foreach ($mealTypes as $mealType) {
        $stmt = $pdo->prepare("INSERT INTO meal_types (company_id, label, active) 
                              VALUES (:company_id, :label, :active)");
        $stmt->execute([
            ':company_id' => $companyId,
            ':label' => $mealType['short_name'],
            ':active' => 1
        ]);
        ...
        $stmt = $pdo->prepare("INSERT INTO meal_types_translation (meal_types_id, language_code, name) 
                              VALUES (:meal_types_id, :language_code, :name)");
        $stmt->execute([
    ...
    for ($i = 0; $i < count($hotelIds); $i++) {
        ...
        foreach ($restaurants as $restaurant) {
            ...
            foreach ($mealTypeIds as $index => $mealTypeId) {
                ...
                // Add pricing times for every day of the week (0-6, Sunday to Saturday)
                for ($day = 0; $day <= 6; $day++) {
                    $stmt = $pdo->prepare("INSERT INTO restaurant_pricing_times 
                                          (restaurant_id, company_id, time, meal_type, reservation_by_room, extra_seats, 
                                          hotel_id, currency_id) 
                                          VALUES (:restaurant_id, :company_id, :time, :meal_type, :reservation_by_room, 
                                          :extra_seats, :hotel_id, :currency_id)");
                    $stmt->execute([
                        ':restaurant_id' => $restaurantId,
                        ':company_id' => $companyId,
                        ':time' => $startTime,
                        ':meal_type' => $mealTypeId, // This field is a foreign key reference
                        ':reservation_by_room' => 1, // Allow reservations by room number
                        ':extra_seats' => 10, // Extra seats available
                        ':hotel_id' => $hotelIds[$i],
                        ':currency_id' => $currencyId
                    ]);
    ...
    for ($i = 0; $i < 5; $i++) {
        ...
        echo "Added restaurant reservation for date $reservationDate at $reservationTime with ID: $reservationId\n";
    }
